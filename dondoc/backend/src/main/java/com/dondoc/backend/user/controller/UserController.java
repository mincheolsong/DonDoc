...
public class UserController {
    ...
    @ApiOperation(value = "비밀번호 찾기(완료)", notes = "비밀번호 찾기 API", response = ApiResult.class)
    ...
    // 회원 정보 변경(우선 테스트)
    @PutMapping("/update")
    @ApiOperation(value = "회원정보 변경(시작안함)", notes = "회원정보 변경 API", response = ApiResult.class)
    ...
    @ApiOperation(value = "프로필 검색(시작안함)", notes = "프로필 검색 API", response = ApiResult.class)
    ...
    // 사용자 검색(친구 추가 또는 모임에서 멤버 초대할때) => 핸드폰 번호로 검색
    @GetMapping("/find_user/{phoneNumber}")
    @ApiOperation(value = "사용자 검색(완료)", notes = "사용자 검색 API", response = ApiResult.class)
    public ApiResult findUser(@PathVariable @ApiParam(value = "핸드폰번호") String phoneNumber){
        try{
            FindUserDto.Response findUserDto = userService.findUser(phoneNumber);
